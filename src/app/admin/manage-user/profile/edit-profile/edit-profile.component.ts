...
export class EditProfileComponent implements OnInit {
  ...
  updateProfile() {
    ...
    this.profileRequest.dateOfBirth = this.profileEditForm.get(
      'dateOfBirth',
    ).value;
  ...
  editPassword() {
    ...
    this.profileRequest.passwordEdit = this.passwordEditForm.get(
      'passwordEdit',
    ).value;

    this.profileService.editPassword(this.profileRequest).subscribe(
      (data) => {
        this.toastrService.showToast('success', 'Thành công', 'Đổi mật khẩu thành công');
        this.toastrService.showToast('warning', 'Thông báo', 'Bạn sẽ phải đăng nhập lại !');
        setTimeout(
          () => (
      ...
      (error) => {
        throwError(error);
        this.toastrService.showToast('danger', 'Thất bại', 'Đổi mật khẩu thất bại');
      },
    );
  }
